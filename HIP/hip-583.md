...
updated: 2022-09-29, 2022-10-03, 2022-10-20, 2022-11-11, 2023-02-23, 2023-03-07
requires: 32
---
...
When an attempt is made to create a new account with `CryptoCreate` with an alias, we check whether the `key alias` has
more than 20 bytes. We then:
...
If we do create a new account from either `CryptoCreate` or `CryptoTransfer`, and the `alias` is an `ECDSA_secp256k1`

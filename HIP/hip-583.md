...
2. `key alias` - This format is created by using the bytes of a simple cryptographic public key supported by Hedera
   ...
   the account owner has asserted cryptographic proof of ownership of the account `alias` key. The key is
   protobuf-encoded and those raw bytes are included in the `alias`.

...
The `FixedFee` message represents a fixed fee charged for each message submission to a topic.
...
The `CustomFee` message defines the type of fee and the account receiving the fee assessed during a message submission to the associated topic. A custom fee may only be fixed and must specify a fee collector account to receive the assessed fees.
...
The `TransactionRecord` message is updated to include the list of custom fees assessed as part of executing the transaction represented by this record.

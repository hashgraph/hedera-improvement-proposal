...
* `MAX_ENTRIES_FOR_FREE_MESSAGES_KEY_LIST = 10`
...
The `ConsensusCreateTopicTransactionBody` message is updated to include the optional Fee Schedule Key, the option Free Message Key List, and the `custom_fees` property for specifying fixed fees during topic creation.
...
The `ConsensusUpdateTopicTransactionBody` message is updated to include the optional Fee Schedule Key, the option Free Message Key List, and the `custom_fees` property for specifying fixed fees during topic creation.
...
* Querying a topic’s allowances and custom fees will be supported via Mirror Node APIs. This can include listing all accounts that have granted allowances to a particular topic. 

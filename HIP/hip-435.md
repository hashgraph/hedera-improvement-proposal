...
`RecordStreamFile` is a newly defined protobuf message which will be used to serialize all `RecordFileObject`s that are part of the same period into record stream files. Periods are intervals of `hedera.recordStream.logPeriod` seconds used to group `RecordFileObject`s into record stream files.
...
The `TransactionSidecarRecord`s created by Hedera nodes will be externalized in `SidecarFile` files grouped by the type of the records. Files will be created using the SidecarFile protobuf holding all the `TransactionSidecarRecords` of a same type (e.g. `ContractStateChange`, `ContractAction`) related to `RecordFileObject`s created for the same period.
...
message SignatureFile {
  ...
  // version of HAPI that was used to serialize this file.
  int fileVersion = 1;

  // Hash of the entire corresponding stream file.
  bytes recordFileHash = 2;

  // A signature object is generated by signing the hash bytes of Entire Hash.
  SignatureObject fileSignature = 3;

  // Metadata Hash of the corresponding stream file.
  bytes metadata = 4; 

  // A signature object is generated by signing the hash bytes of Metadata Hash.
  SignatureObject metadataSignature = 5;
}
```
...
Sidecar Record File 1: 2020-10-19T21_35_39.000000000Z_432_01.rcd
Sidecar Record File 2: 2020-10-19T21_35_39.000000000Z_432_02.rcd
...
Sidecar Record File 1: 2020-10-19T21_35_39.454265000Z_654_01.rcd
Sidecar Record File 2: 2020-10-19T21_35_39.454265000Z_654_02.rcd
Record Signature File(.rcd_sig): 2020-10-19T21_35_39.454265000Z.rcd_sig

All record, sidecar, and signature files will be compressed.

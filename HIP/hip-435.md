...
A `RecordFileObject` consists of a `Transaction` and a `TransactionRecord`, which are already defined protobuf messages. No changes will be required for these messages.
...
```
message SidecarFile {
  repeated TransactionSidecarRecord sidecarRecords = 1;
}
```
...
message SignatureFile {
  SignatureObject fileSignature = 1;
  SignatureObject metadataSignature = 2;
}
```

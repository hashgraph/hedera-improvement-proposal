...
message SignatureFile {
  ...
  // version of HAPI that was used to serialise this file.
  int fileVersion = 1;

  // Hash of the entire corresponding stream file.
  bytes recordFileHash = 2;

  // A signature object generated by signing the hash bytes of Entire Hash.
...
All record, sidecar and signature files will be comressed.
...
The new v6 record file is not backwards compatible with v5 and that clients will need to be updated to support both. Adding support for both concurrently will allow them to ingest historical information and it will allow clients to roll out v6 support ahead of the cutover date.
...
This idea was rejected in order to move to the protobufs format which is more standartized and has native support and grpc compilers for different programming languages.

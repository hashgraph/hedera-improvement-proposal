...
[HIP-412](https://github.com/hashgraph/hedera-improvement-proposal/blob/main/HIP/hip-412.md) normalizes a metadata model for individual NFTs on Hedera where the `metadata` value defined on the individual NFT asset points to a JSON file that itself carries information about the characteristics of the token such as links to media, attribute and property descriptors, creator information, etc.

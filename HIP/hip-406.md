...
At the end of a staking period, there should be an additional record created that indicates a new staking period has started. It will contain the new stake and stakeRewarded for every node, the time and date of the staking period that is now ending, the number of rounds in that staking period, and, for each node, the number of rounds in which it creeated a judge. The record immediately after it will be the record for the first transaction in the new staking period. That is the first transaction with a consensus after midnight UTC, if the staking period is a day.

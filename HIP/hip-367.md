...
```
When associating an account with a token using `HederaTokenStore.associate` which is called when auto assiciating an account to a token during transfer of token units to an associated account,
we validate if this new association [autoAssocitaion] doesnt not increase the number of tokens this account is associated with above the limit enforced by bootstrap property `tokens.maxPerAccount`
```

When using the Account model:
```
When associating an account with a token using `Account.associateWith` which is called when TokenAssociate transaction is called,
we validate if this assocation doesnt not increase the number of tokens this account is associated with above the limit enforced by bootstrap property `tokens.maxPerAccount`
```
...
1. `numAssociations` : The total number of tokens that this account is associated
2. `numZeroBalances` : The total number among these associations which have zero balance
3. `lastAssociation`: The latest association represented as an `EntityNumPair` of account's id and token's id 

...
To support safe direct calls by an EOA in accordance with the security model, a new HRC-755 facade method `<scheduleAddress>.signSchedule()` will be added.
...
Notably, smart contracts may not schedule transactions that are smart contract executions in nature. That is, the scheduling of a `ContractCreate`, `ContractCall` or `EthereumTransaction` will not be not supported by HSS due to the recursive potential.

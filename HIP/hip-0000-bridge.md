...
The best way to successfully integrate a bridge network would be through the community and that is why this HIP is intended for. Using this HIP, the community will have full transperency in both the design and the operations of the bridge.
...
### Terminology
- **Users** - end-users that want to transfer HBAR or HTS tokens from Hedera to Ethereum or Wrapped HBAR and Wrapped Tokens from Ethereum to Hedera
...
- **WHBAR** - ERC20 token issued and operated by Bridge validators. The token represents "wrapped" HBAR on Ethereum. In other words we can say that Hbar is the `native` asset and `WHBAR` is the `non-native` asset.
...
For example transferring 100 HBAR from Hedera to Ethereum (WHBAR) is going to have 1% service fee (1 HBAR) transferred to the Bridge Validators.
...
Alice wants to transfer `HBAR`/`HTS token` from Hedera to Ethereum. She opens any UI that integrates the Bridge and sends the asset to the `Bridge` Account. The memo of the transfer contains the `ethereum-address` which is going to be the receiver of the wrapped asset on Ethereum.
2. **Picking up the Transfer**
The Bridge validator nodes listen for new incoming transfers to the `Bridge` Account. Once they pick up the new transaction, they verify the `state proof` and validate that the `memo` conatins a valid Ethereum address configured as receiver of the wrapped asset.
3. **Paying out fees**
...
3.2 Due to the nature of Scheduled Transactions, only one will be successfully executed, creating a scheduled Entity and all of the others will fail with `TRANSACTION_ALREADY_SCHEDULED` error and the transaction receipt will include the `ScheduleID` of the first submitted transaction.
...
Once supermajority is reached, Alice submits an Ethereum transaction claiming her wrapped asset. The signature contains the raw data signed in the message: `hedera-tx`, `native-token-address`, `receiver` and the `amount` as well as the set of signatures from the Validators.
...
#### Steps
1. **Initiating the Transfer**
Alice wants to transfer her `WHBAR`/`WHTS` tokens from Ethereum to Hedera. She opens any UI that integrates the Bridge and sends `burn` transaction to the `Router` contract. As parameter of the `burn` function, she specifies the Hedera account to receive the wrapped token.
...
#### Transaction Costs
Validators are required to hold `HBAR` in order to pay for their Hedera transactions.

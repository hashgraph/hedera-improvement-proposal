...
Each token key type is mapped to a specific bit in the `uint` field. In order to specify key types, the user will set 
 ...
 * A valid `TokenKey` must have **at least 1** of these bits set and no bit 
higher than those set (this may change in the future if more key types are added). 
...
| `address delegatableContractId` | a smart contract that, if the recipient of the active message frame, should be treated as having signed. |

Note that exactly **one** of these values should be set. Zero or more than 1 will be considered invalid.
...
#### HederaToken
`HederaToken` struct defines basic properties of a Hedera Token, both fungible and non-fungible:
...
| `address treasury`     | The ID of the account which will act as a treasury for the token as a solidity address. This account will receive the specified initial supply or the newly minted NFTs in the case for non-fungible token |
...
| `uint32 maxSupply`     | For tokens of type `FUNGIBLE_COMMON` - the maximum number of tokens that can be in circulation. For tokens of type `NON_FUNGIBLE_UNIQUE` - the maximum number of NFTs (serial numbers) that can be minted. |
...
| `Expiry expiry`        | The `Expiry` struct which defines the expiry schedule of the token.                                                                                                                                        |

Note that the type of token (fungible or non-fungible) is not specified in the `HederaToken` struct - it is implied 
by the specific precompile function that is being called (see the functions below).
...
| `uint32 amount`           | **Populated only when fallback fee is desired.** The amount to charge for the fallback fee                                                |
| `address tokenId`         | **Populated only when fallback fee is desired.** Specifies the fallback fee should be denominated in the fungible token with the given id |
...
The Hbars required to be transferred as value in the precompile call will be equal to the total cost required minus the Hbar cost for the base 100,000 gas cost converted into hbar according to the current gas price at that instance.
...
Since value cannot be transferred through delegate calls, users will not be able to use the token create precompile 
functions via a delegate call. 

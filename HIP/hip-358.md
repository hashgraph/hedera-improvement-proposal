...
A third function will enable updating a token with threshold key values for Admin, KYC, 
...
| ---------- | -------------------------------------------------------------------------------------------------|--------------------------|
|  | `createFungibleToken(HederaToken, uint, uint)`                                                             | `(bool, addess, bytes)`  |
|  | `createFungibleTokenWithCustomFees(HederaToken, uint, uint, FixedFee[], FractionalFee[])`                  | `(bool, addess, bytes)`  |
|  | `createNonFungibleToken(HederaToken, uint, uint)`                                                          | `(bool, addess, bytes)`  |
|  | `createNonFungibleTokenWithCustomFees(HederaToken, uint, uint, FixedFee[], FractionalFee[], RoyaltyFee[])` | `(bool, addess, bytes)`  |

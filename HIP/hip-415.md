...
the record files, since `block hash` is the `32 byte` prefix (out of `48 bytes`) of the running hash of the last
`Record Stream Object` from the previous `Record File` and `timestamp` is `1st Record Stream Object`'s consensus `timestamp`.
...
| baseFeePerGas | Mirror Node | Always zero, since there is no EIP-1559 style floating block capacity fees in Hedera. |
  ...
  | receiptsRoot | N/A | Hardcoded by Mirror Node(s) to hex-encoded 0. It is expected that in the future it will be 
populated once traceability is implemented. |
  ...
  | stateRoot | Mirror Node | Hardcoded by Mirror Node(s) to hex-encoded 0. May contain state proof data in the future |

---
hip: 410 title: Wrapping Ethereum Transaction Bytes in a Hedera Transaction
author: Danno Ferrin <@shemnon>
working-group: Danno Ferrin <@shemnon>, Richard Bair <@rbair23>, Dani Ivanov <
@Daniel_K_Ivanov>
type: Standards Track category: Core needs-council-approval: Yes status: Last
Call last-call-date-time: 2022-04-19T07:00:00Z created: 2022-03-28
...
I am a user, and I have an account with Hedera. I also use MetaMask as my
wallet. By setting up a relay as my network, I can transfer HBAR to smart
contract account.

## Specification

This HIP proposes two new concepts: a transaction body
called `EthereumTransactionBody` that simply wraps the RLP bytes, and a "relay"
that takes RLP bytes and wraps them with an `EthereumTransactionBody` and sends
them to Hedera. The relay can be the client app itself, a backend server, or it
can be any third party relay. Special care is taken to account for payment of
transaction submission separately from payment of transaction execution to be
...
Alice is usually the payer and the sender -- she creates the transaction, signed
...
contract pays fees which include gas fees. Gas is used and then converted into
...
The relay created a standard Hedera transaction and signed it in the standard
...
executing the transaction, including all gas fees and other fees, come from the
Ethereum transaction signed by the user**.

There is one exception. The relay may specify a `maxCoverCharge` which is used
to cover the costs of executing the transaction if, and only if, the Ethereum
transaction cannot cover the costs itself.
...
|----------------------------|-------------|------------------------------|
| `base_fee_per_gas`         | Block       | Base per gas price           |
| `max_priority_fee_per_gas` | Transaction | Tip per gas to pay the miner |
| `max_fee_per_gas`          | Transaction | Max per gas willing to pay   |
| `gas_limit`                | Transaction | Max gas willing to use       |
| `gas_used`                 | Miner       | Amount of gas actually used  |
...
and `gas_used`. The `base_fee_per_gas` is computed dynamically when handling
transactions in such a way as to cover the fees and **no more**.

For example, suppose a transaction requires 1.6M max gas, and the network has a
standard base fee of 50 tinybars per gas, and a standard contract call fee of
.25 HBAR. If called with HAPI, the total cost would be `.25 + 1.6M * .00000005`
or .33 HBAR. If called using an Ethereum transaction, the `base_fee_per_gas`
would be computed such that the net result is the same:

```
1.6M * R = .33
1.6M * R = .25 + 1.6M * .00000005
R = .25/1.6M + .00000005
R = 0.00000020625
```

Thus, `base_fee_per_gas` is `expected_fee/gas_limit + standard_base_gas_fee`
where `expected_fee` is the fee we would have expected to charge had the call
come through HAPI and `standard_base_gas_fee` is the standard gas fee we use for
normal contract execution.

If the computed `base_fee_per_gas` is less than `max_fee_per_gas` then the
transaction will be rejected by the Hedera network. The relay account will
be charged the cost of submitting a transaction, but the sender account will not
be charged.
...
pay and the miner collects all of it.

When Hedera handles a Type 1 transaction, it verifies that `gas_price` is high
enough to cover all fees related to running the node, but **only** collects
those fees that are needed to cover those costs.

Hedera will compute the `base_fee_per_gas` in the same way as it does for
Type 2 transactions defined above. If `base_fee_per_gas` is greater than
`gas_price`, then the transaction will be rejected by the Hedera network.
The relay account will be charged for the cost of submitting a transaction,
but the sender account will not be charged.
...
In cases where the smart contract code is small, and fits in a Hedera
Transaction, the `relay` can create a ContractCreateTransactionBody A new
field, `initcode` will be added to `ContractCreateTransactionBody`. Instead
of uploading a file and then supplying a `fileID`, you can instead supply the
contract code directly in the `ContractCreateTransaction`.
...
Nonces only increase and never go up. Thus, when relays are presented with a
...
line with a regular Hedera transaction and a small fee paid to the relay for the
work of wrapping the transaction and presenting it to the node.

Transaction sent in the Legacy and Type 1 formats will pay all excess fees to
the relay. There is no mechanism to accept less. This can produce awkward

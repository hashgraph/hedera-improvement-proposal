...
This field specifies if the HIP needs to be reviewed and approved by the Hiero Technical Steering Committee before getting an `Accepted` status. This is usually the case for HIPs in the `Standards Track` type and `Core`, `Service`, and `Mirror` categories, but can expand to other HIPs as well, including `Process` HIPs. The HIP author should set it based on their judgment of whether the HIP modifies any of the Hedera Core, Service, or Mirror code, but the HIP editors will double-check if the `Yes` flag needs to be set.

Note: This supercedes the old field `needs-council-approval`. While HIPs prior to Hiero migration maintain this old field for historical referencing; it is no longer used in new HIP headers. Hedera Governing Council TechComm maintains a separate decision-making process on which HIPs to integrate on the Hedera network.

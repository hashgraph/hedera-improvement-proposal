...
HIPs are intended to be the primary mechanism for proposing new features, collecting community input, and documenting the design decisions that have gone into Hedera. The HIP author is responsible for building consensus within the community and documenting dissenting opinions.
...
1. A **Standards Track** HIP describes a new feature or implementation for Hedera. It may also describe an interoperability standard that will be supported outside of the official Hedera node software stack. The Standards Track HIP abstract should include which part of the Hedera ecosystem it addresses. Standards Track HIPs require a specification and a reference implementation:
    ...
    **a. Core:** This includes proposals addressing the Hashgraph algorithm, peer-to-peer networking, and other features implemented on Hedera consensus nodes.

    **b. Service:** Hedera services sit on top of the Core node software and provide the functionalities that users access. These currently include the File Service, Consensus Service, Token Service, and Smart Contract Service. This type of proposal should be used to add to or improve Hedera's service layer. These features are implemented on Hedera consensus nodes.
...
The Technical Steering Committee (the “Hiero TSC”) is responsible for all technical oversight of the Hiero codebase. It has the final say on Core, Service, and Mirror node Standards Track HIPs, as well as Process HIPs. You can read more about Hiero and Hiero TSC here: https://github.com/hiero-ledger/hiero/blob/main/technical-charter.md

### Hedera Council Technical Committee

The Hedera Council, or "the Council," is made up of large organizations that participate in decisions related to the technical development roadmap, treasury management, and general governance. Its Technical Committee (Hedera TechComm) is composed of technical representation of Governing Council members and has the final say on what HIPs will be integrated into the Hedera public network.
...
- __Stagnant__ - Any HIP in `Draft` or `Review,` if inactive for a period of 6 months or greater, is moved to Stagnant. A HIP may be resurrected from this state by Authors or HIP Editors by moving it back to Draft.
- __Rejected__ - Throughout the discussion of a HIP, various ideas will be proposed that are not accepted. Those rejected ideas should be recorded along with the reasoning as to why they were rejected. This helps record the thought process behind the final version of the HIP and prevents people from bringing up the same rejected idea again in subsequent discussions.
- __Last Call__ - This is the final review window for a HIP before moving to "Accepted." A HIP editor will assign the `Last Call` status and set a review end date (`last-call-date-time`), typically 14 days later. If this period results in necessary normative changes, it will revert the HIP to Review.
- __Council Review__ - Some HIPs will have to be approved by the Governing Council before getting an `Accepted` status. This is usually the case for HIPs in the `Standards Track` type and `Core`, `Service` and `Mirror` categories, but can expand to other HIPs as well. The HIP editors will double-check if the `Yes` flag on the `needs-council-approval` header field needs to be set. If HIP needs Governing Council approval, it will have to go through a 'Council Review' status and be reviewed at the next Technical Committee meeting of the Governing Council.
...
3. Motivation -- motivation is critical for HIPs that want to change the Hedera codebase or ecosystem. It should clearly explain why the existing specification is inadequate to address the problem that the HIP solves. HIP submissions without sufficient motivation may be rejected outright.
...
This field specifies if the HIP needs to be reviewed and approved by the Hedera Council Technical Committee before getting a `Accepted` status'. This is usually the case for HIPs in the `Standards Track` type and `Core`, `Service` and `Mirror` categories, but can expand to other HIPs as well. The HIP author should set it based on their judgment of whether the HIP modifies any of the Hedera Core, Service or Mirror code, but the HIP's editors will double-check if the `Yes` flag needs to be set. 

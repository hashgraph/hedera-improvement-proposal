...
4. Calculate the 20-byte `evmAddress` public address value
...
Then migrate all ECDSA accounts
1. Traverse all accounts
2. If `account.key` is not a simple ECDSA key (`evmAddress`) skip
3. If `alias` -> `accountId` map contains `account.key` skip
4. Calculate the 20-byte `evmAddress` public address value
5. Check if `evmAddress` → `accountId` map contains an entry. If present skip

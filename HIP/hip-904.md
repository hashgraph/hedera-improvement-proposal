...
- If an account has `maxAutomaticAssociations` set to max integer value, then we can
  infer that the account holder is willing/interested in receiving unlimited unsolicited tokens in
  their account. In practice having `maxAutomaticAssociations` with max integer value will behave the same
  as with -1 unless the account needs to use values that are bigger than the max integer value.
  Therefore, at this stage we will not implement state migration and the network will leave this setting to be as it is.

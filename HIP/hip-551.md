...
- `batchKey` is not allowed to be set on the batch transaction itself. Any AtomicBatch transaction that has a value set for `batchKey` will be rejected.
...
4. Are deduplicated on an individual basis. If two inner transactions within the same batch have the same transactionID, only one will be processed and the others will be dropped.
...
- The batch transaction will return a single transaction response as usual. If an inner transaction within the batch fails, the response for the batch transaction will contain a response code INNER_TRANSACTION_FAILED. 
...
- The fee for the AtomicBatch outer transactions will be set by CoinComm. The fees for inner transactions will be the same as if they were submitted individually.
...
A new ResponseCodeEnum will be added to the TransactionReceipt to indicate the status of the batch transaction.

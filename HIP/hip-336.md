...
The repeated fields `allowances`, `tokenAllowances` and `nftAllowances` will be added to the `CryptoGetInfoResponse` message. The `allowances`, `tokenAllowances` and `nftAllowances` fields will contain hbar, fungible token and non-fungible token allowances respectively.
...
message CryptoGetInfoResponse {
	...
	message AccountInfo {
		...
		repeated NftAllowance nftAllowances = 21;
	}
...
The `decimals` field, which specifies the number of decimal places the fungible token is divisible by, is returned as a courtesy to the caller so another call to `TokenGetInfo` is not required.
...
This function contains a list of hbar and/or token approval messages. The `CryptoApproval` and `TokenApproval` messages will set an allowance of `amount` fungible tokens or hbar that can be transferred from the owner's account to another account by the `spender` account. The `NftApproval` message sets an allowance for specific NFT serial numbers within the owner's account. Each of the hbar/token approvals in the list do not need to refer to the same spender. The owner can establish different allowances for various spenders in a single transaction.

The total number of approvals contained within the transaction body cannot exceed 20. This number includes all hbar, fungible and non-fungible token approvals. Note that a single `NftApproval` message counts as one approval regardless of the number of serial numbers specified within.
...
In order to grant the `spender` access to *all* of the owner's instances of a particular NFT, the `approvedForAll` field in the `NftApproval` message should be set to `true`.
...
This function will modify a series of existing allowances. The `cryptoApprovals`, `tokenApprovals` and `nftApprovals` fields specify adjustments to crypto allowances, fungible token allowances and non-fungible token allowances respectively.

The `amount` field in the `CryptoApproval` and `TokenApproval` messages is signed, thus this function can be used to increase or decrease an approved allowance.

The `NftApproval` message is used to modify the NFT serial number list for a spender. If the NFT serial number is *positive* then the NFT will be added to the approved list. Conversely if the serial number is *negative* the NFT will be removed from the approved list.
...
message TransactionRecord {
	...
	repeated NftApproval nftAdjustments = 19;
}
```

The `cryptoAdjustments`, `tokenAdjustments` and `nftAdjustments` fields will be populated for `CryptoAdjustApproval` transactions only and will contain the absolute/current value of the altered adjustments.
...
The approval API introduced in the initial proposal only permitted one allowance to be specified via a `oneof`, either an hbar or a token allowance. During internal design reviews, it was felt that since the `oneof` was the only field in the `CryptoApproveAllowanceTransactionBody` message, it would make more sense to separate the `CryptoApproval` and `TokenApproval` messages into distinct transaction body messages. However, doing so would necessitate a similar change to the `CryptoAdjustAllowanceTransactionBody` message to maintain the pattern of separating the hbar and token actions. An alternate solution was proposed to allow `CryptoApproveAllowanceTransactionBody` to specify both hbar and token approvals (as lists) within a single transaction. This approach also permits the caller to specify multiple approvals in bulk rather than issuing many calls.

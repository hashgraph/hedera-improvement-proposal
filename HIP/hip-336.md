...
- recipient: the receiver of tokens when the spender issues a `transferFrom` operation.
...
message CryptoGetInfoResponse {
	...
	message AccountInfo {
		...
		// new fields
		int32 totalAllowances = 19;
		repeated CryptoAllowance allowances = 20;
		repeated TokenAllowance tokenAllowances = 21;
	}
...
The `TokenAllowance` message will contain the allowance information for a single fungible or non-fungible token spendable by the `spender`.

If the token type is non-fungible, the `approvedForAll` field may be set to `true` if the spender has been granted access to all instances of the NFT owned by the owner. Fungible tokens will not `approvedForAll` as this field is only relevant for NFTs.

The `allowances` and `approvedForAll` fields in the `TokenAllowance` message are specified using wrapper types so callers can determine whether the field has a value or not, as opposed to inferring from the type default value (for example, 0 or false).
...
The current number of allowances approved by the owner is specified in `totalAllowances`. This value is calculated as the size of `allowances` plus the size of `tokenAllowances`.
...
The total number of approvals contained within the transaction body cannot exceed 100. This number includes both hbar and token approvals.
...
The transaction must be signed by the owner's account and transactioon fees will also be paid by this account.
...
- Either the `spender` or owner's accounts are frozen.
...
This function will modify an existing `spender`'s allowance by adding `amount` tokens or hbar. Since `amount` is signed, this function can be used to increase or decrease an approved allowance.
...
The transaction fees will be paid from the owner's account and not the spender's. The spender is acting as the owner's proxy in this situation so the responsibility of fee payment rests with the owner.
...
The CryptoGetInfo query will return the same account information as before plus the newly added fields `totalAllowances`, `allowances` and `tokenAllowances`. Clients that are not interested in the account's approved allowances can safely ignore these fields.

...
Describes the addition of Services APIs to approve and exercise allowances to a third party account. The allowances grant another account to transfer hbar, fungible and non-fungible tokens from your account.
...
As a user, I want to enable another user the ability to spend `x` tokens (hbar, fungible and non-fungible tokens) from my account so that they can trade on my behalf without requiring that I expose my private key to them.

As a user, I want to enable another user the ability to spend all instances of a particular NFT that I currently or may in the future hold in my account so that they can trade on my behalf without requiring that I expose my private key to them.
...
The repeated fields `allowances` and `tokenAllowances` will be added to `CryptoGetInfoResponse` message. The `allowances` and `tokenAllowances` fields will contain hbar and token (fungible and non-fungible) allowances respectively.
...
message CryptoGetInfoResponse {
	...
	message AccountInfo {
		...
		// new fields
		repeated CryptoAllowance allowances = 19;
		repeated TokenAllowance tokenAllowances = 20;
	}
...
The `TokenAllowance` message will contain the allowance information for a single fungible or non-fungible token tranferable by the `spender`.

If the token type is non-fungible, the `approvedForAll` field may be set to `true` if the spender has been granted access to all instances of the NFT owned by the owner. Fungible tokens will have an `approvedForAll` value of `false` as this field is only relevant for NFTs. If an NFT has `approvedForAll` set to `true` then `amount` will be not be present.

The `amount` field in the `TokenAllowance` message is specified using a wrapper type so callers can determine whether the field has a value or not, as opposed to inferring from the type default value of `0`.

The `decimals` field, which specifies the number of decimal places the token is divisible by, is returned as a courtesy to the caller so another call to `TokenGetInfo` is not required. This field will have value `0` for NFTs.
...
This function contains a list of hbar and/or token approval messages. Each of these messages will set an allowance of `amount` tokens or hbar that can be transferred from the owner's account to another account by the `spender` account. Each of the hbar/token approvals in the list do not need to refer to the same spender. The owner can establish different allowances for various spenders in a single transaction.

The total number of approvals contained within the transaction body cannot exceed 20. This number includes both hbar and token approvals.
...
It is not an error if either the spender or owner's accounts are frozen or KYC-revoked for the specified token type. If the `spender` tries to transfer a token under these conditions, the transaction will fail anyways.

It is not an error if the token is paused and an allowance approval is submitted.

The transaction must be signed by the owner's account and transaction fees will also be paid by this account.
...
In order to grant the `spender` access to *all* of the owner's instances of a particular NFT, the `approvedForAll` field in the `TokenApproval` message should be set to `true`. However, the `approvedForAll` field cannot be set if `tokenId` refers to a fungible token.
...
- The owner account does not own the specified token.
- `amount` is negative.
- The referenced token is a fungible token and `approvedForAll` is specified.
...
This function will modify a series of existing allowances. Each of the allowance adjustments will add `amount` tokens or hbar to the `spender`'s existing balance. Since `amount` is signed, this function can be used to increase or decrease an approved allowance. Each of the hbar/token approvals in the list do not need to refer to the same spender. The owner can modify different allowances for various spenders in a single transaction.
...
If the `tokenId` field is not populated, the adjustment will be made against the `spender`'s hbar allowance with the owner.
...
- Either the owner, spender or recipient accounts are frozen.
...
The `cryptoAdjustments` and `tokenAdjustments` fields will be populated for `CryptoAdjustApproval` transactions only and will contain the absolute/current value of the altered adjustments.

## Backwards Compatibility

The CryptoGetInfo query will return the same account information as before plus the newly added fields `allowances` and `tokenAllowances`. Clients that are not interested in the account's approved allowances can safely ignore these fields.
...
The approval API introduced in the initial proposal only permitted one allowance to be specified via a `oneof`, either an hbar or a token allowance. During internal design reviews, it was felt that since the `oneof` was the only field in the `CryptoApproveAllowanceTransactionBody` message, it would make more sense to separate the `CryptoApproval` and `TokenApproval` messages into distinct transaction body messages. However, doing so would neccesitate a similar change to the `CryptoAdjustAllowanceTransactionBody` message to maintain the pattern of separating the hbar and token actions. An alternate solution was proposed to allow `CryptoApproveAllowanceTransactionBody` to specify both hbar and token approvals (as lists) within a single transaction. This approach also permits the caller to specify multiple approvals in bulk rather than issuing many calls.

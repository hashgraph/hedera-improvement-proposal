...
to a configureable upper limit, and increasing the limit on the total size of `EthereumTransaction` types from 6KB to a 
configureable upper limit. It also introduces a new throttle bucket that represents the max bytes-per-second for
...
- Permit EthereumTransaction `ethereum_data` size to not exceed a configureable upper limit in KBs
- The entire transaction, including signatures, shall not exceed a configureable upper limit in KBs
...
`ethereum_data` causes the transaction to exceeed the defualt network 6kb limit. The SDK should support an option to 

...
active on a round boundary. Writing the `birthRound` into an event's metadata will provide a way to look up the
...
4. `birthRound` - The latest round to reach consensus on the node when the event was created.
...
payload of the event (the transactions) are not needed. This encapsulation of metadata simplifies the event structure.

...
Allowing for multiple other parents is expected to reduce the time it takes for events to come to consensus.  

Event A `Stronly Sees` event B if there exists paths from A to B in the hashgraph through parent events 
from enough nodes to account for 2/3 of the consensus weight in the roster.  If all events have 2 parents, then the 
minimum possible depth of the longest path contributing to the `Strongly See` relationship is in the order of log2(N) 
where N is the number of nodes in the roster. If all events can have an unbounded number of other parents then it is 
possible to construct parent relationships where this depth can be reduced to length 2.  In practice there are 
several factors that will need to be taken into consideration when choosing parents for events.  This change to the 
serialization format will allow us to explore different algorithms for selecting multiple other parents.    

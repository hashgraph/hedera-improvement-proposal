...
Metadata is typically associated with non-fungible tokens (NFTs), for which metadata like artwork is typically stored off-ledger to avoid the relatively high on-ledger storage costs. HIP-10 normalizes a metadata model for NFTs on Hedera 
where the HTS token points at an off-ledger JSON file that itself carries information about the characteristics of the token and links to media.

The HIP-10 model is

NFT-metadata ----> JSON -----> information & URLs

As for NFTs, it may be useful to provide information about a fungible token beyond that which is stored on-ledger. This specification follows the HIP-10 model to allow a fungible HTS token to point to a JSON file carriny that additional information. 
with a JSON schema optimized for fungible tokens rather than NFTs. 

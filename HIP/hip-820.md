...
When a dApp needs the services of the controller to sign a transaction message and then submit it to the network, it can use the `hedera_signAndExecuteTransaction` method.  This method accepts a base64 encoded protobuf representation of the Hedera API [`TransactionList`](https://github.com/hashgraph/hedera-protobufs/blob/f36e05bd6bf3f572707ca9bb338f5ad6421a4241/sdk/transaction_list.proto) message as input.  The controller decodes the message, interprets the contents, and if valid and approved, appends its signature to the [`SignatureMap`](https://github.com/hashgraph/hedera-protobufs/blob/f36e05bd6bf3f572707ca9bb338f5ad6421a4241/services/basic_types.proto#L780) structure identified as the [sigMap](https://github.com/hashgraph/hedera-protobufs/blob/f36e05bd6bf3f572707ca9bb338f5ad6421a4241/services/transaction_contents.proto#L40C13-L40C13) property of the [`SignedTransaction`](https://github.com/hashgraph/hedera-protobufs/blob/f36e05bd6bf3f572707ca9bb338f5ad6421a4241/services/transaction_contents.proto#L31) It repackages the message as appropriate for transmission to the Hedera network and sends it to the network.  The controller shall wait for the initial response from the Hedera node. In the event of a response resulting in an integer of 0 representing the [`ResponseCodeEnum`](https://github.com/hashgraph/hedera-protobufs/blob/f36e05bd6bf3f572707ca9bb338f5ad6421a4241/services/response_code.proto#L32) signifying the transaction has passed the precheck validations, the controller shall pass back a transaction response as described in [Submit a transaction](https://docs.hedera.com/hedera/sdks-and-apis/sdks/transactions/submit-a-transaction)
...
#### Error
In the event of a precheck validation failure, the controller shall return an error object vs. a result object in the response of the JSON-RPC payload. For more information see <https://www.jsonrpc.org/specification#response_object>.

`code` - the number `-1`

`message` - a string giving a short description of the error, such as "transaction <tranactionId> failed precheck with status 10"

`data` - an integer representing the precheck status code

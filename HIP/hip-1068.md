...
The scopes will include CryptoTransfer, Gas, HTS Token Custom Fees and HAPI Transaction fees.
...
```
A DApp can create the above calls for an EOA using the EOA address as the contract address `<eoaEvmAddress>.selector()`.
This can be debated as a form of account abstraction as it is account specified logic that is run as part of a smart
contract execution.

IHRC1068 methods will also be available via `<IHederaTokenService>.selector()` and
`<IHederaAccountService>.selector()`
<aside>
🚨 **Open Task:** : Add table showing smart contract functions, selectors, hashes and exposed service details.
</aside>
...
A new query param `?scope=(CRYPTO_TRANSFER|GAS|TOKEN_CUSTOM_FEE|TRANSACTION_FEE)` would be added to allow
...
`CRYPTO_TRANSFER` and `TOKEN_CUSTOM_FEE`.
...
A new query param `?scope=(CRYPTO_TRANSFER|TOKEN_CUSTOM_FEE)` would be added to allow
users to filter on the specific applicable scope type.
...
- [x] Q: Should there be a flow to sponsor an infinite amount of HBAR for an account? In one sense this is an infinite
    allowance limited only by the sponsors balance.
    Ans: Yes, this is a valuable flow that simplifies accounting and is based on explicit specification.
- [ ] Q: Could sponsored fees be used to cover gas fees?
    Ans: Conceptually yes. If EOA `A` gives EOA `B` an hbar allowance with a scope gas, then whenever EOA `B` execute a
    smart contract contract the hbar equivalence tof the total gas to be charged to B could be paid for from the
    allowance provided by `A`. Need to confirm the implications here but it seems reasonable and finally opens up the
    zero gas option that we thought we'd have to address later outside of this HIP. What's powerful here is for EVM
    users the allowance can be provided pre onboarding to the network and thus it would be possible for a users to be
    more smoothly welcomed to the network.
    How would EVM tools utilize this flow though using the existing `eth_sendRawTransaction` flow? There's no API
    compliant way to specify per transaction an EOA intent to use their allowance. Could the `eth_sendRawTransaction`
    (as a custom hedera JSON RPC API) and `EthereumTransaction` be extended to capture the intended sporing account?

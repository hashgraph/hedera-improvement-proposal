...
author: [Nana Essilfie-Conduah](https://github.com/Nana-EC)
...
1. `Hedera account id` - `shard.realm.num` format. This is the main account identifier for the network across teh ecosystem.
2. `Hedera account long-form` - This is the bytes of the public cryptographic key of the account appended to the shard and realm in the form `shard.realm.bytes`. Currently, this can only be specified via the `auto-create` flow using the `CryptoTransfer` transaction. 
...
- `Ethereum-native wallet` - wallets that utilize the Ethereum JSON RPC APIs to interact with an Ethereum node. Accounts created via this wallet are ECDSA key based and only expose the account `public-address`.
- `Hedera-native wallet` - wallets that utilize the  Hedera SDK or gRPC HAPI calls to interact with Hedera consensus and mirror nodes. Historically, accounts created via these wallet are `ED25519` based and usually identify the account via the `Hedera account id`.

Post the implementation of [HIP 32](https://hips.hedera.com/hip/hip-32), only accounts created via `auto-create` using the `CryptoTransfer` transaction have aliases set. Wallets and exchanges often utilize the `CryptoCreate` transaction and therefore have not fully integrated alias logic into their UX flows. This limits full applicability of Smart Contract scenarios for Hedera accounts created using `Hedera-native wallet` and web3 tool compatibility for new users coming from external chains using `Ethereum-native wallet`. 
...
The current system presents limited pathways to create an account that is smart contract compatible or identify that account post creation
...
Using this, the ledger will confirm the uniqueness of the alias and create an account that can be referenced via this account public key alias.
...
Notably, most ledgers in the market utilize the `ECDSA` key for accounts, especially where an `EVM` is concerned. In fact to completely support all Smart Contract scenarios on the `EVM` used by Hedera, an account must possess an alias based on its `ECDSA` public key. 
...
- Expand `CryptoCreate` transaction to support setting an alias using a public-key
...
### Account Creation via alias only (Lazy Account Create)

![auto-create-with-address.png](../assets/hip-0000/lazy-create-with-address.png)
...
Where it was limited to CryptoTransfer it will no include CryptoCreate

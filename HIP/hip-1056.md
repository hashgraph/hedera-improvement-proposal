...
- **Redaction**: Redaction refers to the ability of the block stream to to omit data from the stream while retaining full
  verification of the block. This was a goal to support nodes that want to only stores certain types of data but still
  be able to stream a compliant block stream to other consumers who care about the same data but still want to validate
  the stream. This functionality set is reserved for Block Nodes and other non consensus node block stream parsing clients. 
...
> order in which are streamed. The left-to-right order of merkle leaves must be the order in which those items are
> encountered in the stream. Some items are in the "input" subtree and some are in the "output" subtree, within those
> trees the order will be the same as stream-order.
...
weight of the network. In detail, teh state proof is a Merkle Proof created for a value in network consensus state at a
...
block n+1  as well as a live copy of state at the end of block n.
...
[Block Stream Profobuf](../assets/hip-1056/protobuf) and maintained in the public
...
All items in the block other than the final `BlockProof`, `RecordFileItem` and `FilteredItemHash` are either Input or
Output items. This separation is important as they are hashed into two different sub-merkle-trees in the final proof.
...
_file number_ assigned to the item in the this message. The reason they are split into input and output item sets is
...
and is based on the value of a system file 0.0.112. Changes to this file are externalized in the block stream, thus the current
...
- [ ] Q: Should we include a string error message in the `TransactionResult` to give

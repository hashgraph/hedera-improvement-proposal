...
To reconstruct the `EventCore` from an `EventHeader`, replace the parents that are indices with the 
...
transactions in the event, must be serialized together and hashed.  If the `EventHeader` contains no event indices 
...
message EventHeader {
    ...
    /**
     * A list of references to parent events.
     */
...
message ParentEventReference {
    ...
    oneof parent {
        /**
         * The EventDescriptor of the parent event outside the containing block.
         */
        proto.EventDescriptor descriptor = 2;

        /**
         * The index of the parent event within the containing block.
         */
        int64 index = 1;
    }
}

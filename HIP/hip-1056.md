...
> Note: If there are non-transaction state changes (i.e. platform state changes or the sum of all changes to the
transaction receipt queue), those are not a result of the last event's execution or contents. 
They are a consequence of the round as a whole.
...
- [ ] Q: There are some cases where multiple changes made to a singleton state
   ...
   - Actually, it's possible these must be reported multiple times instead of
     ...
     order of updates, and change the state hash. This needs to be verified
...
- [ ] Q: Can we remove `ContractStateChange` from the HSCS transaction outputs as they are potential duplications of
    the platform state changes.
    - This could be possible if platform `StateChange` can highlight the contract entity that is being updated in
    addition to the slot key. `SlotKey` apparently contains both COntractID and contract slot key so this could work.

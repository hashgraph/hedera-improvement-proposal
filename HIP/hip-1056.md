...
To reconstruct the `GossipEvent` from an `EventHeader`, replace the parents that are indices with the 
...
`EventHeader` contains no event indices for parents, the base case is reached and the `GossipEvent` can be constructed 
without a recursive call.  To reconstruct all `EventCore` data in a block, it is simplest to use dynamic 

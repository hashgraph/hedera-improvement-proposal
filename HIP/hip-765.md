...
Similar to NFTs on the network, the fungible token implementation is mainly functional. There is a need for a token creator to specify additional "metadata" about the token. The data and formatting are explored in HIP-YYY, but no field is suitable for storing the link to this metadata. 
